  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <div className="text-[14.5px] font-semibold truncate text-slate-900">
              {p.name}
            </div>
      ...
      <div className="mt-4 pt-3 border-t border-slate-100 flex items-center gap-2">
        <span className="text-[12px] font-medium text-slate-700 group-hover:text-slate-900">
          Open →
        </span>
...
function ExportsHistoryModal({ project, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-[440px] rounded-3xl bg-white shadow-lift-lg p-5 sm:p-6"
      ...
      >
        ...
        {logs && logs.length === 0 && (
          <p className="text-[12.5px] text-slate-500 italic">
            No exports yet. Hit the Export button on this card to create one.
          </p>
        )}
  ...
  return (
    ...
    >
      ...
      <div className="mt-1 text-[12px] text-slate-500">
        Drop a kickoff doc or type{' '}
        <kbd className="font-mono px-1 rounded bg-slate-100">/task</kbd> to seed the plan.
      </div>
...
function NewProjectModal({ onClose, onCreated }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 sm:p-6">
      <div className="w-full max-w-[440px] rounded-3xl bg-white shadow-lift-lg p-5 sm:p-6">
        <div className="flex items-start justify-between mb-4">
          <div>
            <h2 className="text-[18px] font-semibold">New project</h2>
            <p className="text-[12.5px] text-slate-500 mt-0.5">
              You'll be the manager by default.
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            ...
            <input
              ...
              required
              autoFocus
              className="w-full h-12 sm:h-10 rounded-xl border border-slate-200 bg-white px-3 text-[16px] sm:text-[13.5px] focus:outline-none focus:border-orange-400"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-[16px] sm:text-[13.5px] focus:outline-none focus:border-orange-400 resize-none"
            />
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 h-11 sm:h-10 rounded-xl border border-slate-200 text-slate-700 text-[13.5px] font-medium hover:bg-slate-50"
            >
              Cancel
            </button>
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-1 h-11 sm:h-10 rounded-xl bg-orange-500 text-white text-[13.5px] font-medium hover:bg-orange-600 disabled:opacity-60"
            >
...
export default function DashboardClient({ user, projects }) {
    ...
    switch (filter) {
      case 'Mine':
        return projects.filter((p) => (
          p.members.some((m) => m.id === user.id && m.role === 'OWNER')
        ));
      case 'Active':
  ...
  return (
    <div>
      <div className="px-4 sm:px-6 lg:px-8 py-6 pb-28 md:pb-6 max-w-[1400px] mx-auto">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between mb-2">
          <div className="min-w-0">
            ...
            <h1 className="mt-1 text-[28px] sm:text-[28px] leading-tight font-semibold tracking-tight text-slate-900">
          ...
          <div className="flex items-center gap-2 self-start sm:self-auto">
            <button
              type="button"
              className="inline-flex items-center gap-2 px-3 py-2 sm:py-1.5 rounded-lg border border-slate-200 text-slate-700 hover:bg-slate-50 text-[13px]"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-3 py-2 sm:py-1.5 rounded-lg bg-orange-500 text-white text-[13px] font-medium hover:bg-orange-600"
            >
        ...
        <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-3">
          {kpiCards.map((kpi) => (
            <div key={kpi.k} className="rounded-2xl p-4 border bg-white border-slate-200">
              ...
              <div className="mt-1 flex flex-col xs:flex-row sm:flex-row sm:items-baseline gap-1 sm:gap-2">
                <span className="font-mono text-[28px] font-semibold text-slate-900">
                  {kpi.v}
                </span>
        ...
        <div className="mt-8 mb-4">
          <div className="flex items-center justify-center sm:justify-start gap-2 overflow-x-auto">
            <div className="flex shrink-0 items-center p-1 rounded-xl bg-slate-100">
              {FILTERS.map((f) => (
                <button
                  key={f}
                  type="button"
                  onClick={() => setFilter(f)}
                  className={`px-3 py-1 rounded-lg text-[12.5px] font-medium transition-all whitespace-nowrap ${
                    filter === f
            ...
            <div className="hidden sm:block ml-auto whitespace-nowrap text-[12px] font-mono text-slate-500">
              sorted by · recently updated
            </div>
          </div>

          <div className="sm:hidden mt-2 text-center text-[12px] font-mono text-slate-500">
            sorted by · recently updated
          </div>
        </div>
        ...
        {filtered.length === 0 && projects.length === 0 ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map((p) => (
              <ProjectCard key={p.id} p={p} onShowExports={setExportsFor} />
            ))}
          </div>
        )}
      </div>
